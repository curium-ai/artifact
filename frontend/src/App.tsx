...
import { browseUrl, locationFromUrl } from './nav';
  ...
  const handleAction = (action: string, item?: any) => {
    switch (action) {
      ...
      case 'rename':
        ...
        break;
      case 'copy-link': {
        const url = window.location.origin + publicHref(listingPath, item.name);
        navigator.clipboard.writeText(url).catch(() => {});
        toast('Link copied to clipboard');
        break;
      }
      case 'open-tab': {
        window.open(publicHref(listingPath, item.name), '_blank');
        break;
      }
    }
  };
